...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = this.union0(that,this)
  
  private def union0(tail: TweetSet, orig:TweetSet): TweetSet = {
    if(!tail.isEmpty) 
      if(!orig.contains(tail.head))
        union0(tail.tail,orig.incl(tail.head))
      else union0(tail.tail,orig) 
    else orig
  }
    
  // Hint: the method "remove" on TweetSet will be very useful.
  def ascendingByRetweet: Trending = ascHelper(new EmptyTrending())
  def ascHelper(accu:Trending): Trending = {
    if(!this.isEmpty)
      this.remove(this.findMin).ascHelper(accu + this.findMin)
     else
      accu
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter((t=>helper(google,t.text)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter((t=>helper(apple,t.text)))
  ...
  def helper(lst: List[String], str:String): Boolean = {
    if(lst.isEmpty) 
      false 
    else if(str.contains(lst.head)) 
      true 
    else helper(lst.tail, str)
  }
